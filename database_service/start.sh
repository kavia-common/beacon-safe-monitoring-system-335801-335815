...
if [[ ! -f "$PGDATA/PG_VERSION" ]]; then
  ...
  # NOTE: Do not use runuser/chown here; preview environments frequently run as non-root.
  # Postgres only needs a normal user-owned data directory.
...
pg_ctl -D "$PGDATA" -o "-h 0.0.0.0 -p ${PORT}" -w start -l "$LOGFILE"

# Create app role + database (idempotent).
psql -v ON_ERROR_STOP=1 -h 127.0.0.1 -p "${PORT}" -U postgres -d postgres -c "DO \$\$ BEGIN IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'beacon_safe') THEN CREATE ROLE beacon_safe LOGIN; END IF; END \$\$;"
psql -v ON_ERROR_STOP=1 -h 127.0.0.1 -p "${PORT}" -U postgres -d postgres -c "DO \$\$ BEGIN IF NOT EXISTS (SELECT FROM pg_database WHERE datname = 'beacon_safe') THEN CREATE DATABASE beacon_safe OWNER beacon_safe; END IF; END \$\$;"
...
exec postgres -D "$PGDATA" -h "${HOST}" -p "${PORT}"
